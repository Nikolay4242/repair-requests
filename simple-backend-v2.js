...
const app = express();
const port = 3002;  // Изменили порт на 3002
...
// Получить все заявки (публичный endpoint для теста)
  ...
  try {
    ...
    db.get('SELECT COUNT(*) as count FROM users', (err, row) => {
      ...
      if (row.count === 0) {
        ...
        stmt.run(['dispatcher', hash, 'dispatcher', 'Иван Диспетчеров']);
        stmt.run(['master1', hash, 'master', 'Петр Мастеров']);
        stmt.run(['master2', hash, 'master', 'Сергей Ремонтов']);
        stmt.finalize();
...
app.listen(port, () => {
  ...
  console.log(`🔍 Проверка: curl http://localhost:${port}/api/requests/public`);
});
